...
#pragma message "Signal handling is disabled. The application will not be able to stop gracefully."
#include <csignal>
#endif
#include <chrono>
...
/**
 * @brief This class implements a simple VSOMEIP client that sends a request to a
 * VSOMEIP service.
 */
...
public:
    ...
    bool init() {
        ...
        std::cout << "App name: " << app_->get_name()
                  << std::endl;
        ...
        // 每个service_id, instance_id, method_id只能注册一个handler，如果重复注册，会覆盖之前的handler
        ...
        app_->register_availability_handler(RequestResponse_SERVICE_ID, RequestResponse_INSTANCE_ID,
                std::bind(&request_sample::on_availability,
                          this,
    ...
    void stop() {
        ...
        app_->release_service(RequestResponse_SERVICE_ID, RequestResponse_INSTANCE_ID);
    ...
    void on_state(vsomeip::state_type_e _state) {
        if (_state == vsomeip::state_type_e::ST_REGISTERED) {
            std::cout << "Application " << app_->get_name() << " is registered." << std::endl;
            // 如果想要使用任意instance的服务，都需要调用该函数。
            // _service: 服务ID
            // _instance: 实例ID
            // _major：服务的主版本号，默认为ANY_MAJOR
            // _minor：服务的次版本号，默认为ANY_MINOR
            // _use_exclusive_proxy: 是否使用独占代理(即ip和端口其他程序无法使用)，默认为false
            app_->request_service(RequestResponse_SERVICE_ID, RequestResponse_INSTANCE_ID);
        } else {
            std::cout << "Application " << app_->get_name() << " is deregistered." << std::endl;
        }
    }
    ...
    void on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
        ...
        if (RequestResponse_SERVICE_ID == _service && RequestResponse_INSTANCE_ID == _instance) {
    ...
    void on_message(const std::shared_ptr< vsomeip::message > &_response) {
        std::cout << "Received a response from "
                  << "service: " << std::hex << std::setfill('0') << std::setw(4) << _response->get_service()
                  << ", instance: " << std::hex << std::setfill('0') << std::setw(4) << _response->get_instance()
                  << ", client: " << std::hex << std::setfill('0') << std::setw(4) << _response->get_client()
                  << ", method: " << std::hex << std::setfill('0') << std::setw(4) << _response->get_method()
...
int main(int argc, char **argv) {
    ...
    std::string path = "/mnt/workspace/cgz_workspace/Exercise/vsomeip_example/config/request_response.json";
